...
class ChessGUI:
    def __init__(self):
        ...
        # Piece Unicode symbols for better visual representation
        self.piece_symbols = {
            0: '',      # empty
            1: '♜',     # black rook
            2: '♞',     # black knight
    ...
    def square_clicked(self, row, col):
        ...
        if self.game.get_game_state() != "UNFINISHED":
            messagebox.showinfo("Game Over", f"Game is finished! Winner: {self.game.get_game_state()}")
            return
        ...
        if self.selected_square is None:
            ...
            if piece == 0:
                messagebox.showwarning("Invalid Selection", "Please select a piece to move.")
                return
            
            # Check if it's the current player's piece
            if self.game.check_if_valid_player(piece) is False:
                messagebox.showwarning("Invalid Selection", "Please select one of your own pieces.")
                return
            ...
            if self.game.make_move(start_pos, end_pos):
                ...
                # Check for game end
                if self.game.get_game_state() != "UNFINISHED":
                    messagebox.showinfo("Game Over", f"Winner: {self.game.get_game_state()}!")
            else:
    ...
    def update_game_info(self):
        ...
        self.game_state_label.configure(text=f"Game State: {self.game.get_game_state()}")
        self.current_player_label.configure(text=f"Current Player: {self.game.get_current_player()}")
